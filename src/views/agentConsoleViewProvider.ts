...
export class AgentConsoleViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'redqueen.agentConsole';
  private _view?: vscode.WebviewView;
  private memoryEngine = MemoryEngine.getInstance();
      ...
      try {
        ...
        switch (message.type) {
          ...
          case 'FETCH_IMF_SUMMARY': {
            ...
            threads.forEach(t => t.metadata.imfTags.forEach(tag => { tags[tag] = (tags[tag] || 0) as number + 1; }));
          ...
          case 'FETCH_ACTIVE_AGENTS': {
            const agentStatus = this.context.workspaceState.get<Record<string, boolean>>(agentStateKey, {});
          ...
          case 'REQUEST_FILE_LIST': {
            try {
              ...
              const getFiles = async (uri: vscode.Uri) => {
          ...
          case 'SAVE_FILE_CONTENT': {
            try {
              ...
              const editTrace = this.context.workspaceState.get('editTrace', []) as Array<{
                file: string;
                summary: string;
                timestamp: string;
                size: number;
              ...
              // Trigger analysis
              if (this.memoryEngine && typeof (this.memoryEngine as any).analyzeFile === 'function') {
                (this.memoryEngine as any).analyzeFile(path);
              }
  ...
  private getHtmlForWebview(webview: vscode.Webview): string {
    const scriptUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, 'out', 'ui', 'agentConsoleWebview.js')
    );
    return `
      <!DOCTYPE html>
      <html lang="en">
...
async function setConfig(context: vscode.ExtensionContext, config: any) {
