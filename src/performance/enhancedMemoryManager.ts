...
export class EnhancedMemoryManager {
    private readonly MEMORY_PRESSURE_LEVELS = {
        CRITICAL: 0.9,  // 90%
        HIGH: 0.8,      // 80%
        MODERATE: 0.7   // 70%
    };

    private readonly HIGH_IMPACT_EXTENSIONS = [
        'ms-python.python',
    ...
    private _extensionHistory: string[] = [];
    ...
    private async _handleCriticalPressure(): Promise<void> {
    ...
    private async _handleHighPressure(): Promise<void> {
    ...
    private async _handleModeratePressure(): Promise<void> {
    ...
    private async _clearOldCaches(): Promise<void> {
        const now = Date.now();
    ...
    private async _optimizeExtensions(aggressive: boolean): Promise<void> {
        const extensions = vscode.extensions.all;
        const unusedExtensions = await this._findUnusedExtensions();

        // Group extensions by impact
        const highImpactExtensions = extensions.filter(ext =>
            this.HIGH_IMPACT_EXTENSIONS.includes(ext.id)
        );
    ...
    private async _findUnusedExtensions(): Promise<vscode.Extension<any>[]> {
        const extensions = vscode.extensions.all;
    ...
    private async _getExtensionLastUsed(extensionId: string): Promise<number | undefined> {
        const lastUsed = this.context.globalState.get<number>(`extension.${extensionId}.lastUsed`);
        return lastUsed;
    }

    private async _optimizeWorkspaceSettings(aggressive: boolean): Promise<void> {
    ...
    private async _disableNonEssentialFeatures(): Promise<void> {
        const config = vscode.workspace.getConfiguration();
